...
import { TransformerWrapper } from './Transformer';
...
export function FloorCanvas({
  ...
  const safeW = Math.max(1, layout.width);
  const safeH = Math.max(1, layout.height);
  const scale =
    containerWidth > 0 && containerHeight > 0
      ? Math.min(containerWidth / safeW, containerHeight / safeH, 1)
      : 1;
  const offsetX = containerWidth > 0 ? (containerWidth / scale - layout.width) / 2 : 0;
  const offsetY = containerHeight > 0 ? (containerHeight / scale - layout.height) / 2 : 0;
  ...
  const handleTransformEnd = useCallback(
    ...
    [selectedId, onTransformEnd, layout.tables]
  );
