...
export default function CancelReservationPage() {
  ...
  useEffect(() => {
    async function handleCancellation() {
      ...
      if (!token) {
        toast.error('Cancellation token is required');
        router.push('/');
        return;
      }
      ...
      try {
        ...
        setIsLoading(true);
        const data = await cancelReservation(reservationId, token);
        setReservation(data);
        toast.success('Reservation cancelled successfully');
      } catch (err: any) {
        const errorMessage =
          err?.response?.data?.detail || err?.message || 'Failed to cancel reservation';
          ...
          <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <Card variant="elevated">
              <CardHeader className="text-center">
                <CardTitle className="text-2xl text-red-600">Cancellation Failed</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <p className="text-secondary-600 text-center">
                  We couldn&apos;t cancel your reservation. The link may have expired or is invalid.
                </p>
          ...
          <Card variant="elevated">
            ...
            <CardContent className="space-y-4">
              ...
              <p className="text-sm text-secondary-500 text-center pt-4">
                We&apos;re sorry to see you go. If you&apos;d like to make a new reservation, please visit our
                homepage.
              </p>
            </CardContent>
          </Card>
