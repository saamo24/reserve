...
class ReservationService:
    ...
    async def create(self, body: ReservationCreate, guest_id: UUID) -> Reservation:
        ...
        try:
            ...
            # Reload reservation with relations for notifications (branch/table only, guest not needed - identified by phone_number)
            reservation_with_relations = await self._reservation_repo.get_by_id(
                reservation.id, load_branch=True, load_table=True, load_guest=False
            )
